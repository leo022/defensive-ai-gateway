...
def _validated_llm_endpoint(provider: str, endpoint: str, allowed_hosts: list[str]) -> str:
    ...
    if provider == "ollama":
        if host not in _ALLOWED_OLLAMA_HOSTS and host not in allowed:
            raise ValueError(
                "Ollama endpoint is not in the llm.allowed_hosts allowlist; explicitly allow the service host"
            )
        try:
...
class GatewayState:
    ...
    def processing_stats(self) -> dict:
        ...
        stats["queued"] = int(inbox.get("pending", 0)) + int(inbox.get("retry", 0))
        stats["inflight"] = int(inbox.get("processing", 0))
    ...
    def test_llm_connection(self, payload: dict) -> dict:
        ...
        resolved_key = api_key or (saved_key if same_destination else "")
        ...
        try:
            ...
            with urllib.request.build_opener(_NoRedirectHandler()).open(req, timeout=min(timeout, 15)) as resp:
                ...
                return {
                    ...
                    "provider": provider,
                    "endpoint": endpoint,
                    "message": "Gateway 可达，端点返回正常 JSON 响应。",
