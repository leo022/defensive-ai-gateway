...
RASP 证据边界（必须遵守）：
- evidence 中 request_context 的 parameter/body 状态为 present，表示 RASP 已提供该请求字段，且完整原文已在受保护的原始告警中保留。selected_evidence 可能包含命中明确攻击特征的受限字段值，也可能包含不暴露原文的编码载荷元数据。selection_status=selected_by_runtime_correlation 表示内容本身未命中静态特征，但同一 RASP 事件中的规则、hook 字段与危险 sink 已建立运行时相关性；必须结合 content_inspection_status 与 security_relevance 描述为“内容不透明/未识别，但安全相关性由运行时证据支持”，不得声称 RASP 规则未命中。entries 为空且 truncated=false/no_rule_match 仅表示网关投影器未识别到可选值或可关联的编码载荷，绝不能声称是长度、条目、深度或脱敏限制造成，也不得把未入选的业务字段描述成网关丢失。
