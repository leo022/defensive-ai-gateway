...
更新日期：2026-07-31
当前状态：第一阶段 v10 已实现（含控制器优先证据基线、攻击导向 ReAct 调查、原始 Syslog 深读、LLM 最终综合、控制器证据锁定、多域取证与完整调查计时）
...
- v10 重新执行成功并写入新报告时，会在同一事务中删除该 Case 的旧报告及旧报告引用，仅保留最新报告；历史会话、步骤和工具调用继续作为审计轨迹保留。失败、取消或尚未生成报告的重跑不会删除上一份有效报告。
- 部署 v10 后使用“重新执行”生成新会话和 `response-investigation-report-v7` 报告。报告门禁会补齐占位式响应细节，并要求响应动作中每个已知源/目标实体都绑定对应证据。风险加剧与缓解因素会将模型返回的单字符串或异常逐字符数组归一化为完整句子，前端也会兼容修复历史报告的逐字符展示。
- 报告 JSON 契约失败的三次上限按会话持久化计算。若分析员在完整报告失败后暂停并恢复，会话直接从紧凑补丁模式继续，不会重新消耗一次完整报告生成；已经达到上限的会话也不能通过反复恢复绕过有界重试。
- 取证覆盖工具压缩结果时，会优先保留每个工作流的完整证据引用和假设判定所需的核心指标，再用剩余字节容纳采集缺口、请求画像与证据源详情。长诊断字段不能再挤掉引用并导致有证据的支持性假设被报告门禁降级。
