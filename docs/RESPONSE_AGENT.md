...
最新原始分段在 planner/report context 中具有最高优先级。模型继续下一段或进入报告前，必须在公开的短 `rationale` 中保留本段事实摘要；控制器把最近 40 条摘要作为滚动调查笔记传入后续轮次和报告综合。这里只保存可审计事实，不保存隐式思维链。若原始日志在轮次或工具预算内仍未读到 `complete=true`，会话进入明确的预算终态，不能把未完成读取表述为完整取证。
