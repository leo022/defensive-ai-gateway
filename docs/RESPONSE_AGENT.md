...
更新日期：2026-07-30
当前状态：第一阶段 v9 已实现（含控制器优先证据基线、攻击导向 ReAct 调查、原始 Syslog 深读、LLM 最终综合、控制器证据锁定、多域取证与完整调查计时）
...
- v9 重新执行成功并写入新报告时，会在同一事务中删除该 Case 的旧报告及旧报告引用，仅保留最新报告；历史会话、步骤和工具调用继续作为审计轨迹保留。失败、取消或尚未生成报告的重跑不会删除上一份有效报告。
- 部署 v9 后使用“重新执行”生成新会话和 `response-investigation-report-v7` 报告。报告门禁会补齐占位式响应细节，并要求响应动作中每个已知源/目标实体都绑定对应证据。
